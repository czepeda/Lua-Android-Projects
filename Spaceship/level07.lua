...
function scene:createScene(event)
   ...
   -- physics.addBody(spaceship, "dynamic", {density=.1, bounce=0.1, friction=.2, radius=12})
    ...
    mine1 = display.newImage("meteor.png")
    mine1.x = 1000
    mine1.y = 140
    mine1.speed = math.random(6,7)
    mine1.initY = mine1.y
    ...
    screenGroup:insert(mine2)
     ...
     mine3 = display.newImage("alien2.png")
     mine3.x = 500
     mine3.y = 50
     mine3.speed = math.random(6,7)
     mine3.initY = mine3.y
...
function moveMines(self,event)
    if self.x < -50 then
         ...
         self.speed = math.random(7,11)
         self.amp = math.random(25,35)
...
-- Score
score30 = display.newImage('score30.png', 115, 245)
    transition.from(score30, {time = 1000, alpha = 0.1, onComplete = function() timer.performWithDelay(2500, function() transition.to(score30, {time = 200, alpha = 0.1, onComplete = function() display.remove(score30) score30 = nil end}) end) end})
...
function gameOver()
   ...
   storyboard.removeScene("level07")
