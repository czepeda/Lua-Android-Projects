...
function scene:createScene(event)
   ...
   -- physics.addBody(spaceship, "dynamic", {density=.1, bounce=0.1, friction=.2, radius=12})
    ...
    mine1 = display.newImage("alien1.png")
    mine1.x = 1000
    mine1.y = 140
    mine1.speed = math.random(9,10)
    mine1.initY = mine1.y
    ...
    mine2 = display.newImage("alien2.png")
    mine2.x = 800
    mine2.y = 240
    mine2.speed = math.random(10,11)
    mine2.initY = mine2.y
    mine2.amp = math.random(50,100)
    ...
    screenGroup:insert(mine2)
     ...
     mine3 = display.newImage("meteor.png")
     mine3.x = 500
     mine3.y = 50
     mine3.speed = math.random(9,10)
     mine3.initY = mine3.y
...
function moveMines(self,event)
    if self.x < -50 then
         ...
         self.speed = math.random(15,20)
         self.amp = math.random(35,40)
...
ins = display.newImage('ins.png', 130, 270)
    transition.from(ins, {time = 1000, alpha = 0.1, onComplete = function() timer.performWithDelay(4000, function() transition.to(ins, {time = 200, alpha = 0.1, onComplete = function() display.remove(ins) ins = nil end}) end) end})

-- Score
score10 = display.newImage('score20.png', 115, 245)
    transition.from(score10, {time = 1000, alpha = 0.1, onComplete = function() timer.performWithDelay(2500, function() transition.to(score10, {time = 200, alpha = 0.1, onComplete = function() display.remove(score10) score10 = nil end}) end) end})
...
function gameOver()
   ...
   storyboard.removeScene("level011")
